...
void createPic(unsigned char* b, int x, int y, int freqr,int freqb, int freqg)
{
	...
	/* データを生成 */
	for (i = 0; i < y; i++) {
		red = 255 * (unsigned char)(sin(double(freqr) * double(i) / double(10934)) + 1);
		blue = 255 * (unsigned char)(sin(double(freqb) * double(i) / double(10934)) + 1);
		green = 255 * (unsigned char)(sin(double(freqg) * double(i) / double(10934)) + 1);
		/* 1行分のデータを出力 */
		for (j = 0; j < x; j++) {
			*b = blue;
			b++;
